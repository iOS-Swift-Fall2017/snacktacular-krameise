    ...
    func loadData() {
        db.collection("places").getDocuments { (querySnapshot, error) in
            ...
            for document in querySnapshot!.documents {
                ...
                let postingUserID = docData["postingID"] as! String? ?? ""
                let latitude = docData["latitude"] as! CLLocationDegrees ?? 0.0
                let longitude = docData["longitude"] as! CLLocationDegrees ?? 0.0
                let coordindate = CLLocationCoordinate2D(latitude: latitude, longitude: longitude)
                self.places.append(PlaceData(placeName: placeName, address: address, coordinate: coordindate, postingUserID: postingUserID, placeDocumentID: placeDocumentID))
            }
